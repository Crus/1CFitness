...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Знач Отправитель = Неопределено) Экспорт
	...
	ПроверитьПолучениеОбъектовНачальногоОбраза(ЭлементДанных, ПолучениеЭлемента, Отправитель);
...
Функция ПроверитьПолучениеОбъектовНачальногоОбраза(Знач ЭлементДанных, ПолучениеЭлемента, Отправитель)
	...
	Если ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
		...
		Если Отправитель.Метаданные().РаспределеннаяИнформационнаяБаза 
			И Не СозданиеНачальногоОбраза(ЭлементДанных)
			И ЭтоОбъектНачальногоОбразаУзлаРИБ(ОбъектМетаданных(ЭлементДанных)) Тогда
			
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект ""%1"" (%2) входит в состав начального образа (см. служебное событие ПриПолученииОбъектовНачальногоОбразаПланаОбмена)
					|для плана обмена ""%3"", но не исключен из подписок на события регистрации изменений для этого плана обмена.'"), 
				Строка(ЭлементДанных),
				Строка(ОбъектМетаданных(ЭлементДанных)),
				Строка(Отправитель));
		КонецЕсли;
